...
export class SwaprejectedComponent implements OnInit {
  ...
  loadRejectedSwaps(): void {
    ...
    this.shiftService.viewEmployeeRejectedSwapRequests(this.employeeId).subscribe({
      next: (data) => {
        ...
        this.totalRejectedSwaps = this.rejectedSwaps.length;
        this.setPagination();
  ...
  setPagination(): void {
    this.totalPages = Math.ceil(this.totalRejectedSwaps / this.pageSize);
  ...
  changePage(page: number): void {
    if (page >= 1 && page <= this.totalPages) {
      ...
      const endIndex = Math.min(startIndex + this.pageSize, this.totalRejectedSwaps);
      this.pagedRejectedSwaps = this.rejectedSwaps.slice(startIndex, endIndex);
      this.updateVisiblePaginationPages();
    }
  }
