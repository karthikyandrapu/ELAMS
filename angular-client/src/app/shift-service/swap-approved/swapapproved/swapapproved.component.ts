...
export class SwapapprovedComponent implements OnInit {
  ...
  setPagination(): void {
    this.totalPages = Math.ceil(this.totalApprovedSwaps / this.pageSize);
  ...
  changePage(page: number): void {
    if (page >= 1 && page <= this.totalPages) {
      ...
      const endIndex = Math.min(startIndex + this.pageSize, this.totalApprovedSwaps);
      this.pagedApprovedSwaps = this.approvedSwaps.slice(startIndex, endIndex);
      this.updateVisiblePaginationPages();
    }
  }
