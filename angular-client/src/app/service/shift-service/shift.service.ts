...
export class ShiftserviceService {
  ...
  public viewEmployeeRejectedSwapRequests(
    employeeId: number
  ): Observable<Shift[]> {
    const headers = httpOptions.headers.set('role', 'EMPLOYEE');
    return this.http.get<Shift[]>(
  ...
  public viewEmployeeApprovedSwapRequests(
    employeeId: number
  ): Observable<Shift[]> {
    const headers = httpOptions.headers.set('role', 'EMPLOYEE');
    return this.http.get<Shift[]>(
  ...
  public viewEmployeeSwappedWithAnotherEmployee(
    ...
    const headers = httpOptions.headers.set('role', 'EMPLOYEE');
    return this.http.get<Shift[]>(
      `${this.viewEmployeeSwappedWithAnotherEmployeeUrl}/${employeeId}/swap/anotheremployee`,
      { headers }
    );
  }
}
