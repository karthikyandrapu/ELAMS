...
//    private SetDefaultLeaveBalances setDefaultLeaveBalances;
    ...
    public LeaveBalanceDTO getLeaveBalance(Long employeeId, LeaveType leaveType) {
        ...
        System.out.println("gvdjwfwd"+balance.getBalance());
...
//	}
    ...
    public LeaveBalanceDTO createLeaveBalance(Long employeeId, LeaveType leaveType, Double balance) {
        Optional<LeaveBalance> existingBalance = leaveBalanceRepository.findByEmployeeIdAndLeaveType(employeeId, leaveType);
        if (existingBalance.isPresent()) {
            LeaveBalance leaveBalance = existingBalance.get();
    ...
    public List<LeaveBalanceDTO> getAllLeaveBalanceOfEmployee(Long employeeId) {
        ...
        for (LeaveType leaveType : LeaveType.values()) {
            Optional<LeaveBalance> existingBalance = balances.stream()
                                                            .filter(lb -> lb.getLeaveType() == leaveType)
                                                            .findFirst();
            ...
            if (existingBalance.isPresent()) {
                ...
                balance = createDefaultLeaveBalance(employeeId, leaveType);
                balances.add(balance); // Add the newly created default balance to the list
            }
