...
//    }
    ...
    public ResponseEntity<String> deleteAllAttendanceReportsByEmployeeId(@PathVariable Long employeeId) {
        ...
        if (result) {
            ...
            return new ResponseEntity<>("No Attendance Reports found for Employee " + employeeId, HttpStatus.NOT_FOUND);
        }
    }
 ...
 *
    ...
    /**
     * Retrieves employees categorized by their attendance trends, filtered by a manager ID.
     *
     * @param managerIdFromController The manager ID to filter employees by, passed as a path variable.
     * @return A ResponseEntity containing a map where keys are attendance trend categories
     * ("goodAttendance", "averageAttendance", "poorAttendance") and values are lists of employee IDs.
     * Returns HTTP status 200 (OK) with the map in the response body.
     */
    ...
    @GetMapping("/employees-by-trend/{managerIdFromController}")
    public ResponseEntity<Map<String, List<Long>>> getEmployeesByAttendanceTrend(@PathVariable Long managerIdFromController) {
        Map<String, List<Long>> employeesByTrend = attendanceReportService.listEmployeesByAttendanceTrend(managerIdFromController);
