...
public class AttendanceReportServiceImpl implements AttendanceReportService {
   ...
   return dto;
    ...
    private int calculateAbsenteeism(Long employeeId, String dateRange, LeaveType leaveType) {
        ...
        int totalAttendance = calculateTotalAttendance(employeeId, dateRange);
        double leavesTaken = 5.0 - leaveBalance1;

        
        if(leavesTaken < 0) leavesTaken = 0;
        if(leavesTaken > 5) leavesTaken = 5;
    ...
    /**
     * Lists employees categorized by their attendance trends.
     *
     * @return A map where keys are attendance trend categories ("goodAttendance", "averageAttendance", "poorAttendance")
     * and values are lists of employee IDs belonging to those categories.
     */
    @Override
    public Map<String, List<Long>> listEmployeesByAttendanceTrend() {
        List<AttendanceTrends> allTrends = attendanceTrendsRepository.findAll();
        Map<String, List<Long>> employeesByTrend = new HashMap<>();
        employeesByTrend.put("goodAttendance", new ArrayList<>());
        employeesByTrend.put("averageAttendance", new ArrayList<>());
        employeesByTrend.put("poorAttendance", new ArrayList<>());

        if (allTrends != null) { 
            for (AttendanceTrends trend : allTrends) {
                switch (trend.getOverallTrend()) {
                    case "Good Attendance":
                        employeesByTrend.get("goodAttendance").add(trend.getEmployeeId());
                        break;
                    case "Average Attendance":
                        employeesByTrend.get("averageAttendance").add(trend.getEmployeeId());
                        break;
                    case "Poor Attendance":
                        employeesByTrend.get("poorAttendance").add(trend.getEmployeeId());
                        break;
                    default:
