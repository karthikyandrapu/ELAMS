...
@ExtendWith(MockitoExtension.class)
 class LeaveRequestServiceImplTest {
    ...
    @Test
    void getLeaveRequestsByManagerId_NoSubordinates_ReturnsEmptyList() {
        when(employeeClient.getEmployeesByManager(2L)).thenReturn(null); 
        ResponseEntity<List<LeaveRequest>> responseWithNull = leaveRequestService.getLeaveRequestsByManagerId(2L);
        assertEquals(HttpStatus.OK, responseWithNull.getStatusCode());
        assertNotNull(responseWithNull.getBody());
        assertTrue(responseWithNull.getBody().isEmpty());
        when(employeeClient.getEmployeesByManager(2L)).thenReturn(Collections.emptyList()); 
        ResponseEntity<List<LeaveRequest>> responseWithEmptyList = leaveRequestService.getLeaveRequestsByManagerId(2L);
        assertEquals(HttpStatus.OK, responseWithEmptyList.getStatusCode());
        assertNotNull(responseWithEmptyList.getBody());
        assertTrue(responseWithEmptyList.getBody().isEmpty());
